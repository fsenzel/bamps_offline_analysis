...
double offlineHeavyIonCollision::evolveMedium ( const double evolveToTime, bool& _endOfDataFiles )
{
  ...
  while ( ( actiontype != event_endOfCascade ) && ( !stop ) )
  {
    ...
    if ( actiontype == event_newTimestep )
    {
      ...
      if ( actiontype == event_interaction22 )
      {
        ...
        if ( actiontype == event_interaction23 )
        {
          ...
          if ( actiontype == event_interaction32 )
          {
            ...
            if ( actiontype == event_interactionElastic )
            {
              ...
              if ( actiontype == event_particleIdSwap )
              {
...
//           rings.addRates( particles_atTimeNow[(*iIt)] );
    ...
    for ( int j = IXY * etaSliceIndex; j < IXY * ( etaSliceIndex + 1 ); j++ )
    {
      if ( ! ( cells[j].empty() || cellsAdded[j].empty() ) )
      {
          ...
          if ( true )
          {
            ...
            for ( iIt = cells[j].particleList.begin(); iIt != cells[j].particleList.end(); iIt++ )
            {
              ...
              if ( particles_atTimeNow[id].FLAVOR == gluon )
              {
                ...
                if ( particles_atTimeNow[id].FLAVOR == up || particles_atTimeNow[id].FLAVOR == down || particles_atTimeNow[id].FLAVOR == strange || particles_atTimeNow[id].FLAVOR == light_quark )
                {
            ...
            for ( iIt = cellsAdded[j].particleList.begin(); iIt != cellsAdded[j].particleList.end(); iIt++ )
            {
              ...
              addedParticles[id].surroundingPT = rings[nc].getAveraged_p_t();
              if ( addedParticles[id].FLAVOR == gluon )
              {
                ...
                if ( addedParticles[id].FLAVOR == up || addedParticles[id].FLAVOR == down || addedParticles[id].FLAVOR == strange || addedParticles[id].FLAVOR == light_quark )
                {
            ...
            for ( iIt = cells[j].particleList.begin(); iIt != cells[j].particleList.end(); iIt++ )
            {
              ...
              if ( particles_atTimeNow[id].FLAVOR == gluon )
              {
                ...
                if ( particles_atTimeNow[id].FLAVOR == up || particles_atTimeNow[id].FLAVOR == down || particles_atTimeNow[id].FLAVOR == strange || particles_atTimeNow[id].FLAVOR == light_quark )
                {
                  ...
                  if ( particles_atTimeNow[id].FLAVOR == anti_up || particles_atTimeNow[id].FLAVOR == anti_down || particles_atTimeNow[id].FLAVOR == anti_strange || particles_atTimeNow[id].FLAVOR == anti_light_quark )
                  {
            ...
            for ( iIt = cellsAdded[j].particleList.begin(); iIt != cellsAdded[j].particleList.end(); iIt++ )
            {
              ...
              if ( addedParticles[id].FLAVOR == gluon )
              {
                ...
                if ( addedParticles[id].FLAVOR == up || addedParticles[id].FLAVOR == down || addedParticles[id].FLAVOR == strange || addedParticles[id].FLAVOR == light_quark )
                {
                  ...
                  if ( addedParticles[id].FLAVOR == anti_up || addedParticles[id].FLAVOR == anti_down || addedParticles[id].FLAVOR == anti_strange || addedParticles[id].FLAVOR == anti_light_quark )
                  {
        ...
        if ( !cellsAdded[j].empty() ) // belongs to:  if ( !( cells[j].empty() || cellsAdded[j].empty() ) )
        {
          for ( iIt = cellsAdded[j].particleList.begin(); iIt != cellsAdded[j].particleList.end(); iIt++ )
          {
            int iscat = *iIt;
...
//         md2q = md2g * 2.0 / 9.0;
        ...
        if ( pt_addedParticle > 10.0 )
        {
          if ( ParticleOffline::mapToGenericFlavorType ( F2 ) == gluon )
          {
            ...
            if ( ParticleOffline::mapToGenericFlavorType ( F2 ) == light_quark )
            {
              ++n23_collected_quark;
