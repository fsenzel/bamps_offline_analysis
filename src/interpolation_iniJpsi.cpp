...
void interpolation_iniJpsi_dndptdy::configure ( const double sqrtS_arg, const double Bimp_arg, const double sigmaAbs_arg, const double agN, const shadowModelJpsi shadowing_model )
{
  ...
  if ( impact_parameter == 0.0 )
    ...
    if ( impact_parameter == 3.3 )
      name = name + "_b33";
    else
      if ( impact_parameter == 3.6 )
        name = name + "_b36";
      else
        if ( impact_parameter == 4.6 )
          name = name + "_b46";
        else
          if ( impact_parameter == 5.8 )
            name = name + "_b58";
          else
            if ( impact_parameter == 8.2 )
              name = name + "_b82";
            else
              if ( impact_parameter == 9.7 )
                name = name + "_b97";
              else
                if ( impact_parameter == 10.3 )
                  name = name + "_b103";
  ...
  if ( shadowing_model == eps08 )
    ...
    if ( shadowing_model == eps09 )
      name = name + "_eps09";
    else
      if ( shadowing_model == none )
        name = name + "_noshad";
