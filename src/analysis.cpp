...
void analysis::print_spatial_density(const string subfix )
{
  ...
  for ( int i = 0; i < particles_atTimeNow.size(); i++ )
  {
    dndx.add( particles_atTimeNow[i].Pos.X() );
    dndy.add( particles_atTimeNow[i].Pos.Y() );
    dndz.add( particles_atTimeNow[i].Pos.Z() );
  }
...
void analysis::print_dndy_time( int step )
{
  ...
  {
    // normal rapidity
    double normrap = 0.5 * log(( particles_atTimeNow[i].Mom.E() + particles_atTimeNow[i].Mom.Pz() ) / ( particles_atTimeNow[i].Mom.E() - particles_atTimeNow[i].Mom.Pz() ) );
